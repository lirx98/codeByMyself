...
T positionRho(plint iX, plint iY, plint iZ, Params parameters)
{
    Array<T, 3> center(parameters.getCenter());
    T distance2 = (iX - center[0]) * (iX - center[0]) + (iY - center[1]) * (iY - center[1])
                  + (iZ - center[2]) * (iZ - center[2]);
    T radius2 = parameters.getRadius() * parameters.getRadius();
    return distance2 < radius2 ? parameters.getLiquidRho() : parameters.getGasRho();
};
...
public:
    ...
    {
        rho = positionRho(iX, iY, iZ, parameters);
        u[0] = 0.;
        u[1] = 0.;
        u[2] = 0.;
    };
...
public:
    initialTemperature(Params parameters_) : parameters(parameters_) {};
    void operator()(plint iX, plint iY, plint iZ, T &temp, Array<T, 3> &u) const
    {
        Array<T, 3> center(parameters.getCenter());
        T distance2 = (iX - center[0]) * (iX - center[0]) + (iY - center[1]) * (iY - center[1])
                      + (iZ - center[2]) * (iZ - center[2]);
        T radius2 = parameters.getRadius() * parameters.getRadius();
        //temp = iX == 0 || iX == parameters.getNx() - 1 || iY == 0 || iY == parameters.getNy() - 1
        //               || iZ == 0 || iZ == parameters.getNz() - 1
        //           ? 1
        //           : 0.85;
        temp = radius2 < distance2 ? 1. : 0.85;
        u[0] = 0.;
        u[1] = 0.;
        u[2] = 0.;
    }
    ...
    Params &parameters)
{
    ...
    //initializeAtEquilibrium(adLattice, adLattice.getBoundingBox(), initialTemperature(parameters));
    initializeAtEquilibrium(adLattice, adLattice.getBoundingBox(), 0.85, Array<T, 3>(0., 0., 0.));
    setExternalScalar(
        adLattice, adLattice.getBoundingBox(), ADESCRITPOR<T>::ExternalField::scalarBeginsAt, 0.);
    ...
    int iT, Params &parameters)
{
    ...
    plb_ofstream ofile(outName, std::ostream::binary);
    ofile << "variables = x, y, z, rho, u, v, w, temperature" << endl;
    ...
    for (int iZ = 0; iZ < parameters.getNz(); iZ++) {
        for (int iY = 0; iY < parameters.getNy(); iY++) {
            for (int iX = 0; iX < parameters.getNx(); iX++) {
                nsLattice.get(iX, iY, iZ).computeVelocity(velocity);
                ofile << fixed << setprecision(6) << iX << "\t" << iY << "\t" << iZ << "\t"
                      << nsLattice.get(iX, iY, iZ).computeDensity() << "\t" << velocity[0] << "\t"
...
int main(int argc, char *argv[])
{
    ...
    T lx = 1., ly = 1., lz = 1., gasRho = 0.032, liquidRho = 0.279, wallRho = 0.12, radius = 0.1,
      uMax = 0.1;
    Array<T, 3> center(0.5, 0.5, 0.5);
    ...
    pcout << "First Stage Finished" << endl;
    initializeAtEquilibrium(adLattice, adLattice.getBoundingBox(), initialTemperature(parameters));
